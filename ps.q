...
ps:{[t;c;b;a]                                   / partition select
  ...
  v:$[q:0>type b;0;                             / if boolean, 0
    not count b;0;                              /   if empty list, 0
    -11h=type v:first value b;pf~first` vs v;   /   if first grouping is on partition field, 1
    0];                                         /   else, 0
  if[                                           / if..
    $[not count c;0;                            /   if empty list, 0
      type first c;0;                           /     if first constraint is anything other than a list, 0
      -11h=type x:c[0]1;pf~first` vs x;         /     if first constraint is on partition field, 1
      0];                                       /     else, 0
  ...
  if[                                           / if..
    $[count c;0;                                /   if there are constraints on columns other than the partition field, 0
      (g:(value a)~enlist pf)                   /   if aggregation concerns partition field only.. 
        or(value a)~enlist(count;`i)];          /     or count of the virtual column only, 1
    f:key a;                                    /   then name of the aggregated column
    j:dt[d]t;
    if[q;:+f!,$[g;?d@&0<j;,+/j]];
